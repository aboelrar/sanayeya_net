...
//  home_service_detailsDatum.java
...
public class home_service_detailsDatum{
	...
	private home_service_detailsRateUser[] rateUsers;
	...
	public void setRateUsers(home_service_detailsRateUser[] rateUsers){
		this.rateUsers = rateUsers;
	}
	public home_service_detailsRateUser[] getRateUsers(){
	 ...
	 */
	public home_service_detailsDatum(JSONObject jsonObject){
		if(jsonObject == null){
			return;
		}
		...
		if(rateUsersJsonArray != null){
			ArrayList<home_service_detailsRateUser> rateUsersArrayList = new ArrayList<>();
			for (int i = 0; i < rateUsersJsonArray.length(); i++) {
				JSONObject rateUsersObject = rateUsersJsonArray.optJSONObject(i);
				rateUsersArrayList.add(new home_service_detailsRateUser(rateUsersObject));
			}
			rateUsers = (home_service_detailsRateUser[]) rateUsersArrayList.toArray();
		}
	}
	...
	public JSONObject toJsonObject()
	{
		...
		try {
			...
			if(rateUsers != null && rateUsers.length > 0){
				...
				for(home_service_detailsRateUser rateUsersElement : rateUsers){
